...
`INDICE.md` es **generado**. Se regenera con `node indice/scripts/indexar.mjs`; editarlo a mano no sirve de nada.

## 0. Skills invocables y despliegue

El contenido de este repositorio se despliega **suelto dentro del `.claude/skills/` del proyecto anfitrión**. Por tanto cada carpeta de primer nivel con un `SKILL.md` es una skill invocable, y solo eso: no hay stubs ni registro aparte.

| Skill | Enrutador | Para qué |
|---|---|---|
...
1. `SKILL.md` en el primer nivel de la carpeta — nunca en la raíz del repositorio, que al desplegarse sería `.claude/skills/SKILL.md` y no se descubre. Por eso el índice vive en `indice/`.
...
Se ejecuta desde la raíz del repositorio, con Node y sin dependencias. Recorre todo `.md` (excepto `.git/`, `.claude/`, `node_modules/` y `scripts/`), lee la cabecera YAML y los títulos, y produce: la tabla de skills invocables, el inventario por dominio, la tabla de material volátil y una sección **Salud** con enlaces rotos y módulos huérfanos.
...
Al añadir un **dominio nuevo** (una skill más):

1. Crear `<nombre>/SKILL.md` cumpliendo las tres condiciones de la sección 0 — carpeta y `name` iguales, en minúsculas y sin espacios.
2. Añadirlo a la tabla de la sección 0 y a la de la sección 2, y a `DOMINIOS` en `indice/scripts/indexar.mjs` para que salga con nombre legible.
3. Regenerar. `--check` falla si el enrutador no se descubriría al desplegarse.
